...
const Make = () => {
  ...
  return (
    <div className='pt-25 flex flex-col min-h-screen p-8'>
      <Step step={step}/>
      ...
      <button className='btn btn-primary' onClick={handlePrevStep} disabled={step === 1}>上一步</button>
        <button className={step === 5  ? 'btn btn-success' : 'btn btn-primary'} onClick={handleNextStep} disabled={selectedIngredients[`step${step}`]?.length === 0}>
          {step === 5 ? "確認訂單" : "下一步"}
        </button>
      </div>
    </div>
